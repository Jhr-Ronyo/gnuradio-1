  ...
  : d_raw_usrp(_usrp.get()), d_which(which)
{
   ...
   Qu_tx_Ngt3 = 0;               //Enables divide-by-4 freq divider Phase shift
                                 //control bit.  Calculated.
   ...
   Passthru_ref_clk = 0;         //A test mode where the 1 GHz input reference
                                 //is passed directly to the output

   Byp_ram = 1;                  //Bypass the SRAMs

   Dis_adr_dith = 1;             //Disable the dither generator in the ca2adr
                                 //block
                                 
   Dis_p5G_dith = 1;             //Disable the dither generator in the
                                 //lup2decod block

   Byp_fine = 1;                 //Bypass fine delay line control bit

   Exclude32 = 0;                //Bypass fine delay line control bit
                                 //(exclude 32)
   ...
   Thru_ris_en = 0;              //Put rising edges decoders into through-tap
                                 //mode

   Thru_ris_tap_11to6 = 32;      //Through-tap control word
   Thru_ris_tap_5to0 = 0;
   Thru_fal_en = 0;              //Put falling edges decoders into through-tap
                                 //mode

   Thru_fal_tap_11to6 = 32;      //Through-tap control word
   Thru_fal_tap_5to0 = 0;
   Dig_delay = 0;                //This bit provides delay to the clock going
                                 //into the digital block. It is a remnant of 
                                 //past designs and should always be left off 
                                 //because the digClkPhase setting in address
                                 //23 provides much finer control.

   Clk_driver_en = 0;            //This allows the clock to reach the digital
                                 //block. It first passes through the digital/
                                 //analog clock synchronization mux, which means
                                 //that dlEn must be on (dlEn=1) and
                                 //Clk_driver=1 for the digital block to receive
                                 //a clock.  See Byp_fine, address 10, bit 6.

   qu_reg_en = 0;                //This bit enables the voltage regulators that
                                 //supply 1.2 V to all the analog block
                                 //functions. There are 6 separate regulators
                                 //that are simultaneously enabled by this bit.

   qq_reg_en = 0;                //This bit enables the voltage regulators that
                                 //supply 1.2 V to all the Quad Gen functions.
                                 //There are 3 separate regulators that are
                                 //simultaneously enabled by this bit.

   win_rst = 0;                  //When this bit is high, the windowing function
                                 //is in a reset state, which means that no taps
                                 //will be passed to the DDS output regardless
                                 //of the tap select signals coming from the
                                 //digital block.

   fineEn = 0;                   //This bit, when high, routes the coarse taps
                                 //through the fine line before reaching the
                                 //output RS Flip Flop of the DDS. When low,
                                 //the coarse tap is routed directly to the
                                 //output RS Flip Flop.

   fineEnb = 0;                  //Opposite of fineEn

   rsffEn = 0;                   //This bit must be high to send the QuIET 0 and
                                 //180 degree calibration signals off chip. It
                                 //does not control the RS Flip Flop outputs of
                                 //the DDS, though it may have some second order
                                 //(coupling) effect.

   dl_en = 1;                    //Allows the PLL reference to enter the QuIET
                                 //delay line when enabled.

   cp_en = 1;                    //This bit, when enables, activates the charge
                                 //pump that controls the delay line via the
                                 //single pole (one capacitor) DLL loop filter.

   forceCpUpb = 0;               //This bit only matters when pdEn=0 (address
                                 //22, bit 1). When low, the pmos device
                                 //connected to the DLL loop filter cap turns
                                 //on and sources current into the cap, thereby
                                 //increasing the delay line control voltage.

   forceCpDn = 0;                //This bit only matters when pdEn=0 (address
                                 //22, bit 1). When low, the nmos device
                                 //connected to the DLL loop filter cap turns
                                 //off and allows the pmos device to charge up
                                 //the loop cap as described above.

   pdUpTune_1to0 = 3;            //These bits control the pulse width from the
                                 //phase detector into the charge up port of
                                 //the charge pump. 00 turns the charge up
                                 //signal off. 01 is the minimum pulse width
                                 //setting and 11 is the maximum pulse width
                                 //setting.

   pdDnTune_1to0 = 0;            //These bits control the pulse width from the
                                 //phase detector into the charge down port of
                                 //the charge pump. 00 turns the charge down
                                 //signal off. 01 is the minimum pulse width
                                 //setting and 11 is the maximum pulse width
                                 //setting.

   cpUpTune_2to0 = 7;            //These bits control amount of current that is
                                 //sourced while the charge up signal from the
                                 //phase detector is high. 000 is minimum
                                 //current and 111 is maximum current.

   cpDnTune_2to0 = 2;            //These bits control amount of current that is
                                 //sinked while the charge down signal from the
                                 //phase detector is high. 000 is minimum
                                 //current and 111 is maximum current.

   pdEn = 1;                     //When enables, the phase detector will send
                                 //charge up and down signals to the charge pump
                                 //and over ride the forceCpUp and forceCpDn
                                 //settings in address 21. When disabled, the
                                 //forceCpUp and forceCpDn settings will control
                                 //the charge pump.

   digClkPhase_7to0 = 4;         //Only one bit in this field should be active
                                 //at one time. This signal drives a mux that
                                 //selects one of eight clock phases from the
                                 //delay line to drive the digital block. This
                                 //is needed to control the windowing function
                                 //of the DDS.

   Rst_n_async = 0;              //Digital reset
  
   Qg00degDelay_0to4 = 31;       //Adjusts series delay in the 0 degree path for
                                 //the divide-by-two, x1, x2, and x4 quadrature
                                 //generators.

   Qg90degDelay_0to4 = 7;        //Adjusts series delay in the 90 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg180degDelay_0to4 = 31;      //Adjusts series delay in the 180 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg270degDelay_0to4 = 7;       //Adjusts series delay in the 270 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   DischargeTap16_0to3 = 15;     //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling down currents in
                                 //one of the parallel charge pumps.

   ChargeTap16_0to3 = 4;         //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling up currents in one
                                 //of the parallel charge pumps.

   DischargeTapn_0to3 = 15;      //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling down currents in
                                 //one of the parallel charge pumps.

   ChargeTapn16_0to3 = 2;        //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling up currents in one
                                 //of the parallel charge pumps.

   X1sel_32to39 = 0;             //Control for the divide-by-two and x1
                                 //functions.

   X1sel_40to47 = 0;             //Control for the divide-by-two and x1
                                 //functions.
   ...
   X8sel_32to36 = 0;             //Bit 41 is used for a fine line windowing
                                 //control bit. If the fine line is needed, this
                                 //bit needs to be set high if Fout is close to
                                 //Fref (greater than ~ 950 MHz) or for some
                                 //testing modes like pass through or
                                 //thru_rise_en.

   X8sel_41 = 0;                 //hiFout - set for passthrough and Fout close
                                 //to Fref.
   X8sel_37to40 = 0; 
   qutx_fwd_180Cal_en = 0;       //Enables the pad driver that sends the falling
                                 //edge signal off chip. This falling edge
                                 //signal is used internally to trigger the
                                 //'Reset' pin of the output RS Flip Flop.

   qutx_fwd_0Cal_en = 0;         //Enables the pad driver that sends the rising
                                 //edge signal off chip. This rising edge signal
                                 //is used internally to trigger the 'Set' pin
                                 //of the output RS Flip Flop.
   ...
   Qu_tx_Ngt3_2 = 0;             //Enables divide-by-4 freq divider
                                 //Phase shift control bit. Calculated.
   ...
   Dis_adr_dith_2 = 1;           //Disable the dither generator in the ca2adr
                                 //block

   Dis_p5G_dith_2 = 1;           //Disable the dither generator in the
                                 //lup2decod block

   Byp_fine_2 = 1;               //Bypass fine delay line control bit
   Exclude32_2 = 0;              //Bypass fine delay line control bit
                                 //(exclude 32)
   ...
   Thru_ris_en_2 = 0;            //Put rising edges decoders into through-tap
                                 //mode

   Thru_ris_tap_11to6_2 = 32;    //Through-tap mode control word
   Thru_ris_tap_5to0_2 = 0;      //Through-tap mode control word

   Thru_fal_en_2 = 0;            //Put falling edges decoders into through-tap
                                 //mode

   Thru_fal_tap_11to6_2 = 32;    //Through-tap mode control word
   Thru_fal_tap_5to0_2 = 0;      //Through-tap mode control word

   Dig_delay_2 = 0;              //This bit provides delay to the clock going
                                 //into the digital block. It is a remnant of
                                 //past designs and should always be left off
                                 //because the digClkPhase setting in address
                                 //23 provides much finer control.

   Clk_driver_en_2 = 0;          //This bit provides delay to the clock going
                                 //into the digital block. It is a remnant of
                                 //past designs and should always be left off
                                 //because the digClkPhase setting in address
                                 //23 provides much finer control. 
                                 //See Byp_fine, address 10, bit 6.

   qu_reg_en_2 = 0;              //This bit enables the voltage regulators that
                                 //supply 1.2 V to all the analog block
                                 //functions. There are 6 separate regulators
                                 //that are simultaneously enabled by this bit.

   qq_reg_en_2 = 0;              //This bit enables the voltage regulators that
                                 //supply 1.2 V to all the Quad Gen functions.
                                 //There are 3 separate regulators that are
                                 //simultaneously enabled by this bit.

   win_rst_2 = 0;                //When this bit is high, the windowing function
                                 //is in a reset state, which means that no taps
                                 //will be passed to the DDS output regardless
                                 //of the tap select signals coming from the
                                 //digital block.

   fineEn_2 = 0;                 //This bit, when high, routes the coarse taps
                                 //through the fine line before reaching the
                                 //output RS Flip Flop of the DDS. When low,
                                 //the coarse tap is routed directly to the
                                 //output RS Flip Flop.
   ...
   rsffEn_2 = 0;                 //This bit must be high to send the QuIET 0 and
                                 //180 degree calibration signals off chip. It
                                 //does not control the RS Flip Flop outputs of
                                 //the DDS, though it may have some second order
                                 //(coupling) effect.

   dl_en_2 = 1;                  //Allows the PLL reference to enter the QuIET
                                 //delay line when enabled.

   cp_en_2 = 1;                  //This bit, when enables, activates the charge
                                 //pump that controls the delay line via the
                                 //single pole (one capacitor) DLL loop filter.

   forceCpUpb_2 = 0;             //This bit only matters when pdEn=0 (address
                                 //22, bit 1). When low, the pmos device
                                 //connected to the DLL loop filter cap turns on
                                 //and sources current into the cap, thereby
                                 //increasing the delay line control voltage.

   forceCpDn_2 = 0;              //This bit only matters when pdEn=0 (address
                                 //22, bit 1). When low, the nmos device
                                 //connected to the DLL loop filter cap turns
                                 //off and allows the pmos device to charge up
                                 //the loop cap as described above.

   pdUpTune_1to0_2 = 3;          //These bits control the pulse width from the
                                 //phase detector into the charge up port of the
                                 //charge pump. 00 turns the charge up signal
                                 //off. 01 is the minimum pulse width setting
                                 //and 11 is the maximum pulse width setting.

   pdDnTune_1to0_2 = 0 ;         //These bits control the pulse width from the
                                 //phase detector into the charge down port of
                                 //the charge pump. 00 turns the charge down
                                 //signal off. 01 is the minimum pulse width
                                 //setting and 11 is the maximum pulse width
   ...
   cpUpTune_2to0_2 = 7;          //These bits control amount of current that is
                                 //sourced while the charge up signal from the
                                 //phase detector is high. 000 is minimum
                                 //current and 111 is maximum current.

   cpDnTune_2to0_2 = 2;          //These bits control amount of current that is
                                 //sinked while the charge down signal from the
                                 //phase detector is high. 000 is minimum
                                 //current and 111 is maximum current.

   pdEn_2 = 1;                   //When enables, the phase detector will send
                                 //charge up and down signals to the charge pump
                                 //and over ride the forceCpUp and forceCpDn
                                 //settings in address 21. When disabled, the
                                 //forceCpUp and forceCpDn settings will control
   ...
   digClkPhase_7to0_2 = 4;       //Only one bit in this field should be active
                                 //at one time. This signal drives a mux that
                                 //selects one of eight clock phases from the
                                 //delay line to drive the digital block. This
                                 //is needed to control the windowing function
   ...
   Qg00degDelay_0to4_2 = 31;     //Adjusts series delay in the 0 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg90degDelay_0to4_2 = 7;      //Adjusts series delay in the 90 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg180degDelay_0to4_2 = 31;    //Adjusts series delay in the 180 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg270degDelay_0to4_2 = 7;     //Adjusts series delay in the 270 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   DischargeTap16_3to0 = 15;     //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling down currents in
                                 //one of the parallel charge pumps.

   ChargeTap16_3to0 = 4;         //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling up currents in one
                                 //of the parallel charge pumps.

   DischargeTapn_3to0 = 15;      //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling down currents in
                                 //one of the parallel charge pumps. 

   ChargeTapn16_3to0 = 2;        //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling up currents in one
                                 //of the parallel charge pumps.

   X1sel_32to39_2 = 0;           //Control for the divide-by-two and x1
                                 //functions.

   X1sel_40to47_2 = 0;           //Control for the divide-by-two and x1
                                 //functions.
   ...
   X8sel_32to36_2 = 0;           //Bit 41 is used for a fine line windowing
                                 //control bit. If the fine line is needed,
                                 //this bit needs to be set high if Fout is
                                 //close to Fref (greater than ~ 950 MHz) or for
                                 //some testing modes like pass through or
                                 //thru_rise_en.

   X8sel_41_2 = 0;               //hiFout - set for passthrough and Fout close
                                 //to Fref

   X8sel_37to40_2 = 0;
   qutx_fb_180Cal_en = 0;        //Enables the pad driver that sends thefalling
                                 //edge signal off chip. This falling edge
                                 //signal is used internally to trigger the
                                 //'Reset' pin of the output RS Flip Flop.

   qutx_fb_0Cal_en = 0;          //Enables the pad driver that sends the rising
                                 //edge signal off chip. This rising edge signal
                                 //is used internally to trigger the 'Set' pin
                                 //of the output RS Flip Flop.
  ...
  // ------------------------------------
   ...
   Qu_tx_Ngt3_3 = 0;                  //Enables divide-by-4 freq divider
                                      //Phase shift control bit. Calculated.
   ...
   Dis_adr_dith_3 = 1;           //Disable the dither generator in the ca2adr
                                 //block

   Dis_p5G_dith_3 = 1;           //Disable the dither generator in the lup2decod
                                 //block

   Byp_fine_3 = 1;               //Bypass fine delay line control bit
   Exclude32_3 = 0;              //Bypass fine delay line control bit
                                 //(exclude 32)
   ...
   Thru_ris_en_3 = 0;            //Put rising edges decoders into through-tap
                                 //mode

   Thru_ris_tap_11to6_3 = 32;    //Through-tap mode control word
   Thru_ris_tap_5to0_3 = 0;      //Through-tap mode control word

   Thru_fal_en_3 = 0;            //Put falling edges decoders into through-tap
                                 //mode

   Thru_fal_tap_11to6_3 = 0;     //Through-tap mode control word
   Thru_fal_tap_5to0_3 = 0;      //Through-tap mode control word

   Dig_delay_3 = 0;              //This bit provides delay to the clock going
                                 //into the digital block. It is a remnant of
                                 //past designs and should always be left
                                 //offbecause the digClkPhase setting in address
                                 //23 provides much finer control.

   Clk_driver_en_3 = 0;          //This allows the clock to reach the digital
                                 //block. It first passes through the D/A clock
                                 //synchronization mux, which means that dlEn
                                 //must be on (dlEn=1) and Clk_driver=1 for the
                                 //digital block to receive a clock.
                                 //See Byp_fine, address 10, bit 6

   qu_reg_en_3 = 0;              //This bit enables the voltage regulators that
                                 //supply 1.2 V to all the analog block
                                 //functions. There are 6 separate regulators
                                 //that are simultaneously enabled by this bit.

   qq_reg_en_3 = 0;              //This bit enables the voltage regulators that
                                 //supply 1.2 V to all the Quad Gen functions.
                                 //There are 3 separate regulators that are
                                 //simultaneously enabled by this bit.

   win_rst_3 = 0;                //When this bit is high, the windowing function
                                 //is in a reset state, which means that no taps
                                 //will be passed to the DDS output regardless
                                 //of the tap select signals coming from the
                                 //digital block.

   fineEn_3 = 0;                 //This bit, when high, routes the coarse taps
                                 //through the fine line before reaching the
                                 //output RS Flip Flop of the DDS. When low,
                                 //the coarse tap is routed directly to the
                                 //output RS Flip Flop.
   ...
   rsffEn_3 = 0;                 //This bit must be high to send the QuIET 0 and
                                 //180 degree calibration signals off chip. It
                                 //does not control the RS Flip Flop outputs of
                                 //the DDS, though it may have some second order
                                 //(coupling) effect.

   dl_en_3 = 1;                  //Allows the PLL reference to enter the QuIET
                                 //delay line when enabled.

   cp_en_3 = 1;                  //This bit, when enables, activates the charge
                                 //pump that controls the delay line via the
                                 //single pole (one capacitor) DLL loop filter.

   forceCpUpb_3 = 0;             //This bit only matters when pdEn=0 (address
                                 //22, bit 1). When low, the pmos device
                                 //connected to the DLL loop filter cap turns
                                 //on and sources current into the cap, thereby
                                 //increasing the delay line control voltage.

   forceCpDn_3 = 0;              //This bit only matters when pdEn=0 (address
                                 //22, bit 1). When low, the nmos device
                                 //connected to the DLL loop filter cap turns
                                 //off and allows the pmos device to charge up
                                 //the loop cap as described above.

   pdUpTune_1to0_3 = 3;          //These bits control the pulse width from the
                                 //phase detector into the charge up port of the
                                 //charge pump. 00 turns the charge up signal
                                 //off. 01 is the minimum pulse width setting
                                 //and 11 is the maximum pulse width setting.

   pdDnTune_1to0_3 = 1;          //These bits control the pulse width from the
                                 //phase detector into the charge down port of
                                 //the charge pump. 00 turns the charge down
                                 //signal off. 01 is the minimum pulse width
                                 //setting and 11 is the maximum pulse width
   ...
   cpUpTune_2to0_3 = 7;          //These bits control amount of current that is
                                 //sourced while the charge up signal from the
                                 //phase detector is high. 000 is minimum
                                 //current and 111 is maximum current.

   cpDnTune_2to0_3 = 2;          //These bits control amount of current that is
                                 //sinked while the charge down signal from the
                                 //phase detector is high. 000 is minimum
                                 //current and 111 is maximum current.

   pdEn_3 = 1;                   //When enables, thephase detector will send
                                 //charge up and down signals to the charge pump
                                 //and over ride the forceCpUp and forceCpDn
                                 //settings in address 21. When disabled, the
                                 //forceCpUp and forceCpDn settings will control
   ...
   digClkPhase_7to0_3 = 4;       //Only one bit in this field should be active
                                 //at one time. This signal drives a mux that
                                 //selects one of eight clock phases from the
                                 //delay line to drive the digital block. This
                                 //is needed to control the windowing function
   ...
   Qg00degDelay_0to4_3 = 31;     //Adjusts series delay in the 0 degree path for
                                 //the divide-by-two, x1, x2, and x4 quadrature
                                 //generators.

   Qg90degDelay_0to4_3 = 31;     //Adjusts series delay in the 90 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg180degDelay_0to4_3 = 31;    //Adjusts series delay in the 180 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   Qg270degDelay_0to4_3 = 31;    //Adjusts series delay in the 270 degree path
                                 //for the divide-by-two, x1, x2, and x4
                                 //quadrature generators.

   DischargeTap16_0to3_3 = 15;   //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling down currents in
                                 //one of the parallel charge pumps.

   ChargeTap16_0to3_3 = 15;      //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling up currents in
                                 //one of the parallel charge pumps.

   DischargeTapn_0to3_3 = 15;    //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling down currents in
                                 //one of the parallel charge pumps.

   ChargeTapn16_0to3_3 = 15;     //Adjusts DLL offset error in the Quad Gen
                                 //delay line by controlling up currents in one
                                 //of the parallel charge pumps.

   X1sel_32to39_3 = 0;           //Control for the divide-by-two and x1
                                 //functions.

   X1sel_40to47_3 = 0;           //Control for the divide-by-two and x1
                                 //functions.
   ...
   X8sel_32to36_3 = 0;           //Bit 41 is used for a fine line windowing
                                 //control bit. If the fine line is needed, this
                                 //bit needs to be set high if Fout is close to
                                 //Fref (greater than ~ 950 MHz) or for some
                                 //testing modes like pass through or
   ...
   X8sel_41_3 = 0;               //hiFout - set for passthrough and Fout close
                                 //to Fref
   X8sel_37to40_3 = 0;
   qurx_180Cal_en = 0;           //Enables the pad driver that sends the falling
                                 //edge signal off chip. This falling edge
                                 //signal is used internally to trigger the
                                 //'Reset' pin of the output RS Flip Flop.

   qurx_0Cal_en = 0;             //Enables the pad driver that sends the rising
                                 //edge signal off chip. This rising edge signal
                                 //is used internally to trigger the 'Set' pin
                                 //of the output RS Flip Flop.
  ...
  // --------------
   ...
   tx_bb_en = 0;                 //BB Fdbk Mux Buffer BW Control. Enables the
                                 //Forward BB Reference Section of TX

   tx_bb_fdbk_bw = 0;            //Sets the BW of the BB Correction feedback amp

   tx_bb_fdbk_cal_en = 0;        //BB Feedback Mux path Routing. Shorts the BB
                                 //Correction feedback Amp input for
                                 //self-calibration.

   tx_bb_fdbk_cart_err_en = 0;   //Routes the Cartesian error signal through
                                 //the BB Correction feedback.

   tx_bb_fdbk_cart_fb_en = 0;    //Routes the Cartesian feedback signal through
                                 //the BB Correction feedback.

   tx_bb_fdbk_cart_fwd_en = 0;   //Routes the Cartesian reference signal through
                                 //the BB Correction feedback.

   tx_bb_fdbk_en = 0;            //BB Feedback Mux path Routing. Enables the BB
                                 //Correction feedback path via the RX pins.

   tx_bb_fdbk_1q_sel = 0;        //Chooses between I or Q channel for the BB
                                 //Correction feedback path.

   tx_bb_fdbk_lp = 0;            //BB Fdbk Mux Buffer current. Sets the current
                                 //drive capability for BB Correction feedback
                                 //Amp.

   tx_bb_fdbk_statt = 3;         //BB Fdbk Mux Buffer Gain Control. BB Feedback
                                 //Attenuator. Sets the voltage gain for BB
                                 //Correction feedback Amp.

   tx_bb_fdbk_swapi = 0;         //Baseband Feedback Swap I & Ix. Swaps the I
                                 //and Ix BB signals through the BB Correction
                                 //feedback path.

   tx_bb_fdbk_swapq = 0;         //Baseband feedback Swap Q & Qx. Swaps the Q
                                 //and Qx BB signal through the BB Correction
                                 //feedback path.

   tx_bb_gain_cmp = 1;           //Baseband Gain 1 dB Compensation. Adds and
                                 //extra 1.3 dB of Forward Baseband Reference
                                 //Gain

   tx_bb_lp = 0;                 //BB ref. stage current. BB Amp Stage Current.
                                 //Sets the current drive capability for Forward
                                 //BB Reference Amps

   tx_bb_swapi = 1;              //Baseband Swap I & Ix. Swaps the I and Ix BB
                                 //signals through the Forward BB Reference Path

   tx_bb_swapq = 0;              //Baseband Swap Q & Qx. Swaps the Q and Qx BB
                                 //signals through the Forward BB Reference Path

   tx_butt_bw = 0;               //BB ref. Butterworth filter BW control. Sets
                                 //the BW of the Forward BB Reference 4-pole
                                 //Butterworth Filters.

   tx_bw_trck = 5;               //TX MIM cap tracking filter BW. Bandwidth
                                 //Tracking. Sets tracking BW of all the MIM
                                 //cap based TX Filters (16 states).

   tx_cart_en = 0;               //Cartesian FB path Enable.  Enables the
                                 //Cartesian Baseband Section of Tx.

   tx_cart_fb_bb_statt = 15;     //Cartesian down-mix path BB gain. Cartesian
                                 //FB path BB gain. Sets the voltage gain for
                                 //Cartesian BB down converter PMA.

   tx_cart_fb_dcoc_dac_I1 = 32;  //Sets Cartesian BB down converter PMA Dc
                                 //offset correction DAC I1.

   tx_cart_fb_dcoc_dac_I2 = 32;  //Sets Cartesian BB down converter PMA Dc
                                 //offset correction DAC I2.

   tx_cart_fb_dcoc_dac_Q1 = 32;  //Sets Cartesian BB down converter PMA Dc
                                 //offset correction DAC Q1.

   tx_cart_fb_dcoc_dac_Q2 = 32;  //Sets Cartesian BB down converter PMA Dc
                                 //offset correction DAC Q2.
   ...
   tx_cart_zero_statt = 1;       //Cartesian FB path Zero Gain. Sets the
                                 //voltage gain for Cartesian Forward BB Zero
                                 //Amp.

   tx_inbuf_bw = 0;              //Sets the BW of the Forward BB Reference
                                 //Input Buffers.

   tx_inbuf_statt = 0;           //Sets the attenuation of the Forward BB Ref.
                                 //Buffers. 

   tx_output_channel_sel = 0;    //Selects from the 3 RF Forward TX output
                                 //paths, 000 is full power down. 

   tx_p1_bw = 0;                 //Sets the BW of the Cartesian Forward BB Loop
                                 //Pole 1.

   tx_pw_bw1 = 0;                //Cartesian FB path Pole 2 Bandwidth. Sets the
                                 //BW of the Cartesian Forward BB Loop Pole 2.

   tx_p2_bw2 = 0;                //Cartesian FB path Pole 2 Bandwidth. Sets the
                                 //BW of the Cartesian Forward BB Loop Pole 2.
   ...
   tx_rf_fwd_lp = 0;             //RF Forward Bias Reference Control. RF Forward
                                 //Path Current Drain Select. Sets the current
                                 //drive capability for Forward RF Output
                                 //Drivers.

   tx_rf_fwd_statt1 = 0;         //RF Passive Step Attenuator control. RF
                                 //Forward Path Step Attn1. Sets the
                                 //attenuation level for the RF Step attenuators

   tx_rf_fwd_statt2 = 0;         //RF Output Driver Step Attn. Control. RF
                                 //Forward Path Step Attn2. Sets the
                                 //attenuation level for the RF Output Drivers
  ...
  // -----------
   ...
   rx_Idac = 16;                 //I path DCOC DAC setting. Digital values for
                                 //the DC offset adjustment. 11111 represents
                                 //the maximum positive offset adjust and 00000
                                 //represents the maximum negative offset
                                 //adjust. By design, codes 10000 and 01111
                                 //cause no change in the offset voltage.

   rx_dcs = 0;                   //DCOC step size select. Selects the proper
                                 //current reference in the DAC to maintain
                                 //constant step size at ouptut of baseband
                                 //filters. This value works in tandem with the
                                 //BiQuad Gain Select (address 198, bits 4:3) to
                                 //maintain a constant step size of 24 mV at
                                 //filter output.

   rx_den = 0;                   //Enables the DC offset correction DACs in the
                                 //I and Q path.
                                 
   rx_Qdac = 12;                 //Q path DCOC DAC setting. Digital values for
                                 //the DC offset adjustment. 11111 represents
                                 //the maximum positive offset adjust and 00000
                                 //represents the maximum negative offset
                                 //adjust. By design, codes 10000 and 01111
   ...
   rx_cmpen = 0;                 //Enables the DC offset correction comparator
                                 //used in the DCOC circuitry.
                                 
   rx_dcoc = 0;                  //Enables the DC offset correction circuitry
                                 //for automatic correction of the DC offset in
                                 //the baseband filters.
                                 
   rx_ten = 0;                   //Enables the RC tuning circuit to tune RX and
                                 //TX baseband filters.
                                 
   rx_ren = 0;                   //Enables the ramp circuit used in the RC
                                 //tuning circuitry to tune the RX and TX
                                 //baseband filters.
                                 
   rx_dven = 0;                  
   rx_dv = 0;                    //DCOC/tune clock divider select. Selects the
                                 //clock rate used for clocking the DCOC and RC
                                 //tuning circuit. Bits 3 and 2 set the divider
                                 //setting used for both the DCOC circuitry and
                                 //RC Tune circuitry. Bits 1 and 0 set the
                                 //divider setting for the dedicated divider in
                                 //the DCOC circuitry. Table below shows the
                                 //mapping of divider settings. The DCOC clock
                                 //divider setting is the total divide ratio of
                                 //both dividers. The maximum divide ratio is
                                 //8*8 = 64.
                                 
   rx_extc = 0;                  //Enables the external capacitor pins to allow
                                 //for external low-frequency pole to be placed
                                 //in the signal path between the mixer and
                                 //baseband filter.
                                 
   rx_cen = 0;                   //Chopper enable for filter stages. Settings to
                                 //enable which amplifier the clock is being
                                 //applied.
   ...
   rx_fen = 0;                   //Enables baseband filters. 0 puts filter in
                                 //power save mode.
                                 
   rx_onchen = 0;                //Enables on-channel detector.
   rx_offchen = 0;               //Enables off-channel detector.
                                 
   rx_foe = 0;                   //Enables the output of the baseband filters.
                                 //Otherwise the baseband filter outputs are in
                                 //a Hi-Z state to allow transmitter to use
                                 //filter output pins. When Filter Enable is
                                 //set LOW, outputs are disabled (Hi-Z).
                                 
   rx_offch = 1;                 //Sets the Clip Threshold for the Off-channel
                                 //Detector.

   rx_onchf = 0;                 //Sets the Fade Threshold for the On-channel
                                 //Detector relative to the On-channel clip
                                 //point.
                                 
   rx_onchc = 2 ;                //Sets the Clip Threshold for the On-channel
                                 //Detector.
   ...
   rx_rq = 1;                    //Sets the BiQuad filter resistor value. The
                                 //natural frequency of the BiQuad (wo) is this
                                 //resistor value multiplied by the BiQuad
                                 //Capacitor value.
                                 
   rx_rv = 1;                    //Sets the VGA filter (passive filter)
                                 //resistor value. The pole frequency of the
                                 //passive filter is this resistor value
                                 //multiplied by the VGA capacitor value.
                                 
   rx_rip = 0;                   //Sets the MPA input resistor value that sets
                                 //the gain of the PMA. Gain of the PMA is
                                 //Rf/Rin where Rf is the PMA feedback resistor
                                 //and Rin is the input resistor. 
                                 //Note that the input resistance remains at
                                 //2 kohm differential for all settings. An R2R
                                 //ladder is used to accomplish this while
                                 //changing the Rin value.
                                 
   rx_rfp = 2;                   //Sets the PMA feedback resistor value that
                                 //sets the gain of the PMA as well as the pole
                                 //frequency (along with PMA capacitor value). 
                                 //Gain of the PMA is Rf/Rin where Rf is the PMA
                                 //feedback resistor and Rin is the input
                                 //resistor.

   rx_cp_12to8 = 0;              //Sets the PMA filter capacitor value. The
                                 //pole frequency of the PMA filter is the PMA
                                 //feedback resistor value multiplied by this
                                 //Capacitor value.
                                 //PMA Capacitor (in pF) = (PMAC) * 0.0625 + 1

   rx_gs = 0;                    //Sets the gain of the VGA in the baseband
                                 //filter

   rx_cp_7to0 = 0;               //PMA cap select LSBs. Sets the PMA filter
                                 //capacitor value. The pole frequency of the
                                 //PMA filter is the PMA feedback resistor value
                                 //multiplied by this Capacitor value.
                                 //PMA Capacitor (in pF) = (PMAC) * 0.0625 + 1

   rx_cv_10to3 = 0;              //VGA cap select MSBs. Sets the VGA (passive)
                                 //filter capacitor value. This pole frequency
                                 //of the passive filter is the VGA resistor
                                 //value multiplied by this Capacitor value.
                                 //VGA Capacitor (in pF) = (VGAC) * 0.0625 + 1

   rx_cv_2to0 = 0;               //VGA cap select LSBs. Sets the VGA (passive)
                                 //filter capacitor value. This pole frequency
                                 //of the passive filter is the VGA resistor
                                 //value multiplied by this Capacitor value.
                                 //VGA Capacitor (in pF) = (VGAC) * 0.0625 + 1

   rx_cc_2to0 = 0;               //Compensation control. Disables additional
                                 //compensation capacitance in the VGA and
                                 //BiQuad op-amps to allow for higher
                                 //bandwidths. Also increases the op-amp
                                 //dominate pole-frequency which improves filter
                                 //response. Bit 4 controls the VGA amplifier,
                                 //Bit 3 controls the feedback amplifier in the
                                 //BiQuad, and Bit 2 controls the output buffer
                                 //in the BiQuad.

   rx_cq_9to8 = 0;               //BiQuad cap select MSBs. Sets the BiQuad
                                 //filter capacitor value. The natural frequency
                                 //of the BiQuad (wo) is the BiQuad resistor
                                 //value multiplied by this Capacitor value.
                                 //BiQuad Capacitor (in pF) = 
                                 //  (BiQuadC) * 0.125 + 2

   rx_cq_7to0 = 0;               //BiQuad cap select LSBs. Sets the BiQuad
                                 //filter capacitor value. The natural frequency
                                 //of the BiQuad (wo) is the BiQuad resistor
                                 //value multiplied by this Capacitor value.
                                 //BiQuad Capacitor (in pF) = 
   ...
   rx_rxchen = 0;                //RX mixer enable. Must be set to 1 to enable
                                 //Mixer operation.

   rx_bbq_div2or4 = 0;           //Selects divide ratio of RX Quad Gen when
                                 //using external LO.  0->DIV2, 1 ->DIV1
   rx_Loselect = 0;              //RX external LO select.  Enables external LO
                                 //clock source.
   ...
   vagenable2 = 1;               //Enable VAG Gen in Full On Mode (fast ramp
                                 //from sleep)
  ...
  //-------------------
   ...
   TestMuxSetting = 0;           //Four Output Description
...
rfic::shutdown()
{
  ...
  // For more information about setting each variable and SPI register, see
  // RFIC4 SPI Default Variables.xls 
...
rfic::send_reg(int reg_no,int dat )
{
    ...
    //since the 14 bit address lies from bits 1-14..0th bit is for auto
    //increment..15th bit is for read/write
    int hdr = int((reg_no << 1) & 0x7fff);     char c[1];
    c[0] = (char)(dat&0xff);
...
{
    // Returns a vector containing the information in the first 320 registers
    // in integer form
    //
    // dat is the data to write to register 0
    // spi_enable is the spi enables
    ...
    //Starts reading from register 0 since the previous write is on register 0
...
void rfic::calc_freq_vars(double _Fclk,double _Fout,int ret_arr[])
{
  ...
  //@type Fclk: float
  //@param Fout: Desired clock frequency for one of three freq synthesizers (Hz)
  //@type Fout: float
  //
  // Calculate RFIC register variables to set frequency of freq synthesizers
...
void rfic::calc_phase_vars(double _Fclk, double _Fout,double phsh,int ret_arr[])
{
  ...
  //@type _Fclk: float
  //@param _Fout: Desired clock freq for one of three freq synthesizers (Hz)
  //@type _Fout: float
  //@param phsh: Desired phase shift in degrees
  //@type phsh: float
  //
  // Calculate RFIC register variables to set phase of freq synthesizers
  ...
  int temp, Ngt_phsh, RorFrNpRdiv4_17to10_phsh, RorFrNpRdiv4_9to2_phsh;
  int RorFrNpRdiv4_25to18_phsh, RorFrNpRdiv4_1to0_phsh;
...
struct freq_result_t rfic::set_rx_freq(double target_freq)
{
  ...
  args.baseband_freq = target_freq;
  return args;
  //FIXME -- How do I know if the RFIC successfully attained 
...
int rfic::RSSI_clip()
{
  ...
  rx_onchf = 0; //Sets the Fade Threshold for the On-channel Detector relative
                //to the On-channel clip point.
